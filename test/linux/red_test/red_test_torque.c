...
struct TorqueIn *val1 = NULL;
struct TorqueOut *target1 = NULL;
...
void redtest(char *ifname)
{
   ...
   if (ec_init(ifname))
   {
      ...
      if (ec_config_init(FALSE) > 0)
      {
         ...
         for (int i = 1; i <= ec_slavecount; i++)
         {
            WRITE(i, 0x6060, 0, buf8, 10, "OpMode"); // Operation mode torque
         ...
         for (int i = 1; i <= ec_slavecount; i++)
         {
            ...
            WRITE(i, 0x1c12, 1, buf32, 0x1602, "assign 1601");
            WRITE(i, 0x1c12, 0, buf8, 1, "enable RX");

            WRITE(i, 0x1c13, 0, buf8, 0, "disable TX");
            WRITE(i, 0x1c13, 1, buf32, 0x1a02, "assign 1A03");
   ...
   while (1)
   {
      ...
      if (dorun > 0)
      {
         ...
         if (wkc >= expectedWKC && val1 && target1)
         {
            ...
            {
               target1->torque = 5000;
               target1->torque *= blink? -1:1;
               if (counter % 10000 == 0)
                  blink = !blink;

               printf("slave 1 --> counter %d, torque: %d\n", counter, target1->torque);
