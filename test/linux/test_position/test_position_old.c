    ...
    if (ec_init(ifname))
    {
        ...
        if (ec_config_init(FALSE) > 0)
        {
            ...
            for (int i = 1; i <= ec_slavecount; i++)
            {
                ...
                WRITE(i, 0x1c12, 1, buf32, 0x1600, "assign 1600");
                WRITE(i, 0x1c12, 0, buf8, 1, "enable RX");

                WRITE(i, 0x1c13, 0, buf8, 0, "disable TX");
                WRITE(i, 0x1c13, 1, buf32, 0x1a00, "assign 1A00");
            ...
            if (ec_slave[0].state == EC_STATE_OPERATIONAL)
            {
                ...
                for (int i = 1; i <= ec_slavecount; i++)
                {
                    READ(i, 0x1c12, 0, buf8, "rxPDO:0");
                    READ(i, 0x1c13, 0, buf8, "txPDO:0");
                ...
                for (i = 1; i <= 10000; i++)
                {
                    ...
                    if (wkc >= expectedWKC)
                    {
                        ...
                        { // Fault + Fault reaction active
                            ...
                            READ(1, 0x1003, 0, buf32, "errore letto");
                            int max_err = buf32;
